...
export function drawLevelUpScreen(ctx, canvas, player, gameState) {
  ...
  const title = gameState.state === 'chest' ? 'LOOT CHEST' : 'LEVEL UP!'
  const subtitle = gameState.state === 'chest' ? 'Choose 1 upgrade card' : `Level ${player.level}`
  ...
  ctx.fillText(subtitle, cx, cy - 100)
  ctx.restore()
  ...
  choices.forEach((upgrade, i) => {
    ...
    const y = cy - 80
    const color = RARITY_COLORS[upgrade.rarity] || '#ffffff'
    const highlight = upgrade.rarity === 'epic' || upgrade.rarity === 'legendary'
    ...
    if (highlight || upgrade.rarity === 'rare') {
      ctx.shadowBlur = upgrade.rarity === 'legendary' ? 28 : 14
    ...
    ctx.textAlign = 'center'
    ctx.fillStyle = color
    ctx.fillText(upgrade.icon, x + CARD_W / 2, y + 50)
  ...
  ctx.fillText('Press a number key or click a card', cx, cy + 160)
  ctx.restore()
}
